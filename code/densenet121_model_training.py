...
def get_run_directory():
    ...
    return f"deepfake_densenet121_{timestamp}"
...
class DeepFakeDetector(nn.Module):
    def __init__(self, freeze_backbone=True):
        ...
        # Load the DenseNet-121 modell
        self.densenet121 = models.densenet121(weights=DenseNet121_Weights.IMAGENET1K_V1)

        if freeze_backbone:
            # Freeze the feature extraction layers
            for param in self.densenet121.features.parameters():
                param.requires_grad = False

        # Modify the fully connected layer to include custom layers
        dropout_rate = 0.5
        num_classes = 2  # Binary classification

        # Replace the classifier (fully connected layer) with custom layers
        # The DenseNet-121 classifier input size is 1024 (default for ImageNet)
        num_ftrs = self.densenet121.classifier.in_features
        self.densenet121.classifier = nn.Sequential(
            nn.Linear(num_ftrs, 1024),  # First layer
            nn.ReLU(inplace=True),
            nn.LayerNorm(1024),  # Layer Normalization
            nn.Dropout(dropout_rate),  # Dropout for regularization
            nn.Linear(1024, 512),  # Second layer
            nn.ReLU(inplace=True),
            nn.LayerNorm(512),  # Additional Layer Normalization
            nn.Dropout(dropout_rate),  # Dropout again
            nn.Linear(512, num_classes),  # Output layer
        )

        # Initialize weights of the added layers
    ...
    def _initialize_weights(self):
        ...
        for m in self.densenet121.classifier:
