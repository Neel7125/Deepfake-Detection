...
def get_run_directory():
    ...
    return f"deepfake_xceptionnet_{timestamp}"
...
class DeepFakeDetector(nn.Module):
    def __init__(self, freeze_backbone=True, num_classes=2):
        ...
        # Load XceptionNet model with pretrained weights
        self.xception = create_model("xception", pretrained=True, num_classes=0)  # no classifier
        
        if freeze_backbone:
            for param in self.xception.parameters():
                param.requires_grad = False

        num_ftrs = self.xception.num_features  # Typically 2048
        dropout_rate = 0.5
        self.classifier = nn.Sequential(
            ...
            nn.Dropout(dropout_rate),
            nn.Linear(256, num_classes)  # Output layer with 2 logits (for binary classification)
        )
