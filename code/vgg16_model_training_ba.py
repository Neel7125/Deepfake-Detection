...
def get_run_directory():
    ...
    return f"deepfake_vgg16_batchnorm{timestamp}"
...
class DeepFakeDetector(nn.Module):
    def __init__(self, freeze_backbone=True):
        ...
        self.vgg16.classifier = nn.Sequential(
            ...
            nn.BatchNorm1d(4096),  # Batch Normalization for 4096 units
            nn.Dropout(dropout_rate),  # Dropout for regularization
            nn.Linear(4096, 2048),  # Second fully connected layer
            nn.ReLU(inplace=True),
            nn.BatchNorm1d(2048),  # Batch Normalization for 2048 units
            nn.Dropout(dropout_rate),  # Dropout again
            nn.Linear(2048, 1024),  # Third fully connected layer
            nn.ReLU(inplace=True),
            nn.BatchNorm1d(1024),  # Batch Normalization for 1024 units
            nn.Dropout(dropout_rate),  # Dropout again
